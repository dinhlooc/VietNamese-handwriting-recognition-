...
FIXED_OUTPUT_IMAGE = os.path.join(output_dir, 'camera_ocr_result.jpg')
FIXED_OUTPUT_TEXT = os.path.join(output_dir, 'camera_ocr_result.txt')
...
def process_frame(frame, predictor, ocr):
    ...
    for i, (x1, y1, x2, y2) in enumerate(line_boxes):
        ...
        cv2.putText(visual_img, f"{i+1}: {text}", (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 
                   0.7, (0, 0, 255), 2)
    
    return visual_img, results

def main():
    parser = argparse.ArgumentParser(description='Realtime OCR from camera')
    parser.add_argument('-c', '--camera', type=int, default=0, 
                        help='Camera index (default: 0, usually webcam)')
    ...
    print("Initializing camera-based OCR system...")
    config = load_config()
    ...
    try:
        ...
        print("\nCamera OCR ready!")
        print("Press 'SPACE' to capture and recognize text")
        print("Press 'q' to quit")
        print(f"Results will be saved to fixed files:")
        print(f"  - Image: {FIXED_OUTPUT_IMAGE}")
        print(f"  - Text:  {FIXED_OUTPUT_TEXT}")
        ...
        while True:
            ...
            if key == 32:  # Space key
                print("\nProcessing frame for text recognition...")
                ...
                if result_img is not None:
                    ...
                    print("No text found or error processing frame")
